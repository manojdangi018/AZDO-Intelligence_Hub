...
function initCredentials() {
  ...
  const backend = typeof isBackendMode === 'function' && isBackendMode();
  const patInput = document.getElementById('targetPat');
  const patLabel = patInput?.closest('.field-group')?.querySelector('label');
  const patHelp = patInput?.closest('.field-group')?.querySelector('small');
  if (backend && patInput) {
    patInput.value = '';
    patInput.disabled = true;
    patInput.placeholder = 'Managed securely by Cloudflare Worker';
    patInput.classList.add('bg-slate-100', 'cursor-not-allowed');
    if (patLabel) patLabel.innerHTML = 'Personal Access Token <span class="text-emerald-600">(Backend)</span>';
    if (patHelp) patHelp.textContent = 'The Azure DevOps PAT is stored as a Cloudflare Worker secret and is never sent to this browser.';
  }
  handleOrgChange();
}
...
function credentialsReady() {
  if (typeof isBackendMode === 'function' && isBackendMode()) return true;
  return Boolean(document.getElementById('targetPat')?.value.trim());
}
...
async function loadProjectsList() {
  ...
  if (!credentialsReady()) return showModal('Please enter your Personal Access Token (PAT).', 'targetPat');
...
function disconnectSession() {
  ...
  document.getElementById('pipelineSummaryTableBody').innerHTML = `<tr><td colspan="8" class="p-4 text-center text-slate-400">Click "Fetch Pipeline Runs" to scan pipeline definitions.</td></tr>`;
  document.getElementById('pipelineTableBody').innerHTML = `<tr><td colspan="6" class="p-4 text-center text-slate-400">No build runs loaded.</td></tr>`;
