async function fetchUserAccessData() {
  ...
  try {
    ...
    setStatus(`Loaded ${accessRows.length} member assignments across all ${Object.keys(groupMemberCounts).length} groups & teams.${accessScanErrors ? ` ${accessScanErrors} group/team scan(s) failed or were denied.` : ''}`, accessScanErrors ? 'info' : 'success');
