async function fetchWorkItemsData() {
  ...
  try {
    ...
    if (wiIds.length === 0) {
      document.getElementById('workItemsTableBody').innerHTML = `<tr><td colspan="6" class="p-4 text-center text-slate-400">No work items found in project "${escapeHtml(project)}".</td></tr>`;
    ...
    const idChunks = [];
    for (let i = 0; i < wiIds.length; i += 200) idChunks.push(wiIds.slice(i, i + 200));

    const workItems = [];
    const WORK_ITEM_BATCH_CONCURRENCY = 4;
    for (let i = 0; i < idChunks.length; i += WORK_ITEM_BATCH_CONCURRENCY) {
      const batch = idChunks.slice(i, i + WORK_ITEM_BATCH_CONCURRENCY);
      const results = await Promise.all(batch.map(ids => {
        const detailsUrl = `https://dev.azure.com/${org}/${encodeURIComponent(project)}/_apis/wit/workitems?ids=${ids.join(',')}&fields=${fields}&api-version=7.0`;
        return fetchAzDo(detailsUrl, authHeader);
      }));
      results.forEach(data => workItems.push(...(data?.value || [])));
    }
...
function renderWorkItemsTableBatch(append = false) {
  ...
  const html = nextBatch.map((r, rowIndex) => `
    <tr class="hover:bg-slate-50 transition" data-detail-type="work-item" data-detail-index="${batchStartIndex + rowIndex}">
      ...
      <td class="p-4"><span class="px-2 py-0.5 rounded text-xs font-semibold bg-slate-100 text-slate-700">${escapeHtml(r.type)}</span></td>
      <td class="p-4 font-medium text-slate-900 max-w-sm truncate" title="${escapeHtml(r.title)}">${escapeHtml(r.title)}</td>
      <td class="p-4 text-xs font-semibold ${r.assignedTo === 'Unassigned' ? 'text-slate-400 italic' : 'text-slate-800'}">${escapeHtml(r.assignedTo)}</td>
      <td class="p-4 text-xs">
        ...
        }">${escapeHtml(r.state)}</span>
      </td>
      <td class="p-4 text-xs text-slate-500">${escapeHtml(r.createdDate)}</td>
    </tr>
  `).join('');
