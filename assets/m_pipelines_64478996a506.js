function normalizePipelineStatus(value) {
  ...
  if (['succeeded', 'success', 'passed'].includes(raw)) return 'succeeded';
  if (['failed', 'failure'].includes(raw)) return 'failed';
  if (['canceled', 'cancelled'].includes(raw)) return 'canceled';
  if (['partiallysucceeded', 'partiallysucceeded', 'partial', 'partially-succeeded'].includes(raw)) return 'partiallySucceeded';
  if (['inprogress', 'in-progress', 'running'].includes(raw)) return 'inProgress';
  if (['notstarted', 'queued'].includes(raw)) return raw === 'queued' ? 'queued' : 'notStarted';
  return raw || 'unknown';
}
function formatPipelineStatus(value) {
  const map = { succeeded: 'Succeeded', failed: 'Failed', canceled: 'Canceled', partiallySucceeded: 'Partially Succeeded', inProgress: 'In Progress', queued: 'Queued', notStarted: 'Not Started', unknown: 'Unknown' };
  return map[normalizePipelineStatus(value)] || String(value || 'Unknown');
}
      ...
      await Promise.all(batch.map(async (pipe) => {
        try {
          ...
          runsObtained.forEach(b => {
            const result = normalizePipelineStatus(b.result || b.status || 'unknown');
            ...
            if (result === 'succeeded') {
              ...
              summaryMap[pipe.name].canceled++;
            } else if (result === 'partiallySucceeded') {
              summaryMap[pipe.name].partiallySucceeded++;
            } else {
              summaryMap[pipe.name].other++;
            }
            ...
            allRuns.push({
              ...
              reason: trigger,
              author: author,
              result: normalizePipelineStatus(b.result || b.status || 'unknown'),
...
function renderPipelineSummaryTableBatch(append = false) {
  ...
  if (rawStore.pipelineSummaries.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" class="p-4 text-center text-slate-400">No pipelines found.</td></tr>`;
  ...
  const html = nextBatch.map((p, rowIndex) => {
    ...
    return `
      <tr class="hover:bg-slate-50 transition" data-detail-type="pipeline-summary" data-detail-index="${batchStartIndex + rowIndex}">
        <td class="p-4 font-semibold text-slate-900">${escapeHtml(p.name)}</td>
        ...
        <td class="p-4"><span class="px-2 py-0.5 rounded text-xs font-semibold bg-amber-50 text-amber-700">${p.canceled}</span></td>
...
function renderPipelineTableBatch(append = false) {
  ...
  if (rawStore.pipelines.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" class="p-4 text-center text-slate-400">No recent build runs found for scanned pipelines.</td></tr>`;
    container.classList.add('hidden');
    return;
  }
  ...
  const html = nextBatch.map((r, rowIndex) => `
    <tr class="hover:bg-slate-50 transition" data-detail-type="pipeline-run" data-detail-index="${batchStartIndex + rowIndex}">
      <td class="p-4 font-semibold text-slate-900">${escapeHtml(r.name)}</td>
      <td class="p-4 text-xs font-mono text-slate-700 font-semibold">${escapeHtml(r.branch)}</td>
      <td class="p-4 font-mono text-xs text-blue-600 font-bold">#${escapeHtml(r.buildNumber)}</td>
      <td class="p-4 text-xs font-medium text-slate-800">${escapeHtml(r.author)}</td>
      <td class="p-4 text-xs text-slate-500">${escapeHtml(r.triggerDate)}</td>
      <td class="p-4">
        <span class="px-2 py-0.5 rounded-full text-xs font-semibold ${
          r.result === 'succeeded' ? 'bg-emerald-100 text-emerald-700' :
          r.result === 'failed' ? 'bg-red-100 text-red-700' :
          r.result === 'canceled' ? 'bg-slate-200 text-slate-700' : 'bg-amber-100 text-amber-700'
        }">${escapeHtml(formatPipelineStatus(r.result))}</span>
      </td>
    </tr>
  `).join('');
...
function exportPipelinesToXLSX() {
  ...
  const summaryData = rawStore.pipelineSummaries.map(p => ({
    ...
    "Partially Succeeded": p.partiallySucceeded || 0,
    "Other": p.other || 0,
  ...
  const runsData = (rawStore.pipelines || []).map(r => ({
    ...
    "Result": formatPipelineStatus(r.result),
    "Trigger Date": r.triggerDate,
