  ...
  const html = nextBatch.map((b, rowIndex) => {
    ...
    const policiesHtml =
      ...
      (b.minReviewers > 0
        ...
        : '') +
      b.policies.map(p =>
        `<span class="bg-blue-50 text-blue-700 border border-blue-200 text-[10px] font-semibold px-1.5 py-0.5 rounded">${escapeHtml(p)}</span>`
      ).join('') +
      `</div>`;

    return `
      <tr class="hover:bg-slate-50 transition" data-detail-type="policy-branch" data-detail-index="${rawStore.repos.indexOf(b)}">
        <td class="p-4 font-semibold text-slate-900">${escapeHtml(b.repo)}</td>
        <td class="p-4">
        ...
        <td class="p-4 text-xs font-medium text-slate-700">${escapeHtml(b.author || 'Unknown')}</td>
        <td class="p-4 text-xs text-slate-500">${escapeHtml(b.date || 'N/A')}</td>
      </tr>
    `;
  }).join('');
