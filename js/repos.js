...
async function fetchBranchPolicies(project, repoId, branchName) {
  ...
  const request = (async () => {
    const gitPolicyUrl = `${encodeURIComponent(project)}/_apis/git/policy/configurations?repositoryId=${encodeURIComponent(repoId)}&refName=${encodeURIComponent(refName)}&$top=1000&api-version=7.1`;

    try {
      const data = await fetchAzDo(gitPolicyUrl);
      const policies = Array.isArray(data?.value) ? data.value : [];
      if (policies.length > 0) return policies;
    } catch (err) {}

    const fallbackUrl = `${encodeURIComponent(project)}/_apis/policy/configurations?$top=1000&api-version=7.1-preview.1`;

    try {
      const data = await fetchAzDo(fallbackUrl);
      const allPolicies = Array.isArray(data?.value) ? data.value : [];
      const currentRepo = String(repoId).toLowerCase();
      const currentRef = refName.toLowerCase();

      return allPolicies.filter(policy => {
        if (!policy || policy.isDeleted === true || policy.isEnabled === false) return false;
        const scopes = Array.isArray(policy.settings?.scope) ? policy.settings.scope : [];
        if (scopes.length === 0) return true;

        return scopes.some(scope => {
          const scopeRepo = String(scope.repositoryId || '').toLowerCase();
          const scopeRef = String(scope.refName || '').toLowerCase();
          if (scopeRepo && scopeRepo !== currentRepo) return false;
          if (!scopeRef || scopeRef === 'refs/heads/*' || scopeRef === '*') return true;
          const matchKind = String(scope.matchKind || 'Exact').toLowerCase();
          if (matchKind === 'defaultbranch') return true;
          if (matchKind === 'prefix') return currentRef.startsWith(scopeRef);
  ...
  window.__azdoBranchPolicyCache.set(cacheKey, request);
  return request;
}
...
function parsePolicyInformation(policies) {
  ...
  (policies || []).forEach(policy => {
    if (!policy || policy.isDeleted || policy.isEnabled === false) return;
    const typeId = (policy.type?.id || '').toLowerCase();
    const typeName = (policy.type?.displayName || '').trim();
    const typeNameLower = typeName.toLowerCase();
    const settings = policy.settings || {};

    if (typeId === POLICY_TYPES.MIN_REVIEWERS) {
      const count = Number(settings.minimumApproverCount) || 1;
      minReviewers = Math.max(minReviewers, count);
      policyList.push(`${count} Required Reviewer${count > 1 ? 's' : ''}`);
      return;
    }

    if (typeId === POLICY_TYPES.BUILD || typeNameLower.includes('build')) {
      policyList.push(settings.displayName || 'Build Validation');
      return;
    }

    if (typeId === POLICY_TYPES.REQUIRED_REVIEWERS || typeNameLower.includes('required reviewer')) {
      policyList.push('Required Reviewers');
      return;
    }

    if (typeId === POLICY_TYPES.WORK_ITEM_LINKING || typeNameLower.includes('work item')) {
      policyList.push('Work Item Linking');
      return;
    }

    if (typeId === POLICY_TYPES.COMMENT_RESOLUTION || typeNameLower.includes('comment')) {
      policyList.push('Comment Resolution');
      return;
    }

    if (typeId === POLICY_TYPES.MERGE_STRATEGY || typeNameLower.includes('merge strategy')) {
      policyList.push('Merge Strategy');
      return;
    }

    if (typeId === POLICY_TYPES.STATUS_CHECK || typeNameLower.includes('status')) {
      policyList.push(typeName || 'Status Check');
      return;
    }

    policyList.push(typeName || 'Branch Policy');
  });

  return {
    hasPolicy: policyList.length > 0,
...
function ensurePolicyTableHeaders() {
  const branchTable = document.getElementById('table-repos');
  const prTable = document.getElementById('table-repo-prs');

  if (branchTable) {
    const headerRow = branchTable.querySelector('thead tr');
    if (headerRow) {
      headerRow.innerHTML = `
  ...
  if (prTable) {
    const headerRow = prTable.querySelector('thead tr');
    if (headerRow) {
      headerRow.innerHTML = `
        <th class="p-4">Repository</th>
...
async function fetchRepositoryData() {
  ...
  startFetching('Fetching branches, branch policies, and PR telemetry across selected repository...');
  ...
  if (rawInput !== '-- All Repositories --' && rawInput !== '__ALL__') {
    ...
    targetRepos = exactMatches.length > 0 ? exactMatches : cachedRepos.filter(r => r.name.toLowerCase().includes(rawInput.toLowerCase()));
  }
  ...
  try {
    const repoPromises = targetRepos.map(async r => {
      ...
      const prUrl = `${project}/_apis/git/repositories/${r.id}/pullrequests?searchCriteria.status=all&$top=100&api-version=${API_VERSION}`;
      ...
      if (refsPromise.status === 'fulfilled' && refsPromise.value) {
        ...
        branchDetails = await Promise.all(
          refs.map(async ref => {
            const bName = ref.name.replace(/^refs\/heads\//, '');
            const commitUrl = `${project}/_apis/git/repositories/${r.id}/commits?searchCriteria.itemVersion.version=${encodeURIComponent(bName)}&searchCriteria.itemVersion.versionType=branch&$top=1&api-version=${API_VERSION}`;

            const [commitResult, policyResult] = await Promise.allSettled([
              fetchAzDo(commitUrl),
              fetchBranchPolicies(project, r.id, bName)
            ]);

            const commitData = commitResult.status === 'fulfilled' ? commitResult.value : null;
            const policies = policyResult.status === 'fulfilled' ? policyResult.value : [];
            const policyInfo = parsePolicyInformation(policies);
            const topCommit = commitData?.value?.[0] || null;
      ...
      if (prsPromise.status === 'fulfilled' && prsPromise.value) {
        ...
        const prRows = await Promise.all(
          prList.map(async pr => {
            ...
            const policies = await fetchBranchPolicies(project, r.id, targetBranch);
            const policyInfo = parsePolicyInformation(policies);
    ...
    renderPolicyBranchesTableBatch(false);
    ...
    const branchPolicyCount = rawStore.repos.filter(b => b.hasPolicy).length;
    const prPolicyCount = allPRs.filter(p => p.targetPolicies && p.targetPolicies.length > 0).length;

    setStatus(`Loaded ${rawStore.repos.length} branches, ${branchPolicyCount} branches with policies, ${allPRs.length} pull requests across ${targetRepos.length} repositories.`, 'success');
    stopFetching();
  } catch (err) {
    setStatus(`Error fetching branches and policies: ${err.message}`, 'error');
    stopFetching();
  }
}
  ...
  const html = nextBatch.map(b => {
    const policiesHtml = b.hasPolicy
      ? `<div class="flex flex-wrap gap-1 max-w-xs">
          ${b.minReviewers > 0 ? `<span class="bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] font-bold px-1.5 py-0.5 rounded">${b.minReviewers} Min Reviewer${b.minReviewers > 1 ? 's' : ''}</span>` : ''}
    ...
    return `
      <tr class="hover:bg-slate-50 transition">
        ...
        <td class="p-4">${b.isStale ? '<span class="bg-amber-100 text-amber-700 text-xs px-2 py-0.5 rounded-full font-semibold">Stale</span>' : '<span class="bg-emerald-100 text-emerald-700 text-xs px-2 py-0.5 rounded-full font-semibold">Active</span>'}</td>
...
function getPolicyBranches() {
  if (!Array.isArray(rawStore.repos)) return [];
  return rawStore.repos.filter(b => b.hasPolicy === true && Array.isArray(b.policies) && b.policies.length > 0);
}

function renderPolicyBranchesTableBatch(append = false) {
  const tbody = document.getElementById('policyBranchesTableBody');
  const container = document.getElementById('seeMorePolicyBranchesContainer');
  const remainingEl = document.getElementById('policyBranchesRemainingCount');
  if (!tbody || !container || !remainingEl) return;

  const policyBranches = getPolicyBranches();
  if (!append) {
    tbody.innerHTML = '';
    rawStore.policyBranchesIndex = 0;
  }
  ...
  if (policyBranches.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" class="p-4 text-center text-slate-400">No branches with policies found.</td></tr>`;
    container.classList.add('hidden');
    remainingEl.textContent = '0';
    return;
  }

  const nextBatch = policyBranches.slice(rawStore.policyBranchesIndex, rawStore.policyBranchesIndex + PAGE_SIZE);
  rawStore.policyBranchesIndex += nextBatch.length;

  const html = nextBatch.map(b => {
    const policiesHtml = `<div class="flex flex-wrap gap-1 max-w-xl">${b.minReviewers > 0 ? `<span class="bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] font-bold px-1.5 py-0.5 rounded">${b.minReviewers} Required Reviewer${b.minReviewers > 1 ? 's' : ''}</span>` : ''}${b.policies.map(p => `<span class="bg-blue-50 text-blue-700 border border-blue-200 text-[10px] font-semibold px-1.5 py-0.5 rounded">${p}</span>`).join('')}</div>`;

    return `
      <tr class="hover:bg-slate-50 transition">
        <td class="p-4 font-semibold text-slate-900">${b.repo}</td>
        <td class="p-4"><span class="font-mono text-xs text-blue-700 bg-blue-50 px-2 py-0.5 rounded font-semibold">${b.branch}</span></td>
        <td class="p-4"><span class="bg-indigo-50 text-indigo-700 border border-indigo-200 text-xs font-bold px-2 py-1 rounded">${b.minReviewers > 0 ? b.minReviewers : '0'}</span></td>
        <td class="p-4">${policiesHtml}</td>
  ...
  const remaining = policyBranches.length - rawStore.policyBranchesIndex;
  if (remaining > 0) {
    container.classList.remove('hidden');
...
function exportPolicyBranchesToXLSX() {
  const policyBranches = getPolicyBranches();
  if (policyBranches.length === 0) {
    if (typeof showModal === 'function') showModal('No branches with policies are available to export.');
    return;
  }
  ...
  exportToExcelFile({ "Branches With Policies": data }, "AzureDevOps_Branches_With_Policies");
}
...
function renderRepoPrsTableBatch(append = false) {
  ...
  const html = nextBatch.map(pr => {
    ...
    if (pr.minRequiredReviewers > 0) {
      policyBadge = `<div class="flex flex-wrap gap-1 max-w-xs"><span class="bg-indigo-50 text-indigo-700 border border-indigo-200 text-xs font-bold px-2 py-0.5 rounded">${pr.minRequiredReviewers} Min Required (${pr.reviewersCount} Assigned)</span>${pr.targetPolicies.filter(p => !p.toLowerCase().includes('required reviewer')).map(p => `<span class="bg-blue-50 text-blue-700 border border-blue-200 text-xs font-semibold px-2 py-0.5 rounded">${p}</span>`).join('')}</div>`;
    } else if (pr.targetPolicies && pr.targetPolicies.length > 0) {
      policyBadge = `<div class="flex flex-wrap gap-1 max-w-xs">${pr.targetPolicies.map(p => `<span class="bg-blue-50 text-blue-700 border border-blue-200 text-xs font-semibold px-2 py-0.5 rounded">${p}</span>`).join('')}</div>`;
    } else {
      policyBadge = `<span class="text-xs text-slate-400">${pr.reviewersCount > 0 ? pr.reviewersCount + ' Reviewer(s)' : 'No Policies Set'}</span>`;
    }
    ...
    return `
      <tr class="hover:bg-slate-50 transition">
        ...
        <td class="p-4"><span class="px-2 py-0.5 rounded-full text-xs font-semibold ${pr.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : pr.status === 'active' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600'}">${pr.status}</span></td>
...
function exportBranchesToXLSX() {
  ...
  const data = rawStore.repos.map(b => ({
    ...
    "Branch Policies": b.policies ? b.policies.join(', ') : 'None',
    "Required Reviewers": b.minReviewers || 0,
...
function exportRepoPrsToXLSX() {
  ...
  const data = rawStore.repoPrs.map(p => ({
    ...
    "Target Branch Policies": p.targetPolicies ? p.targetPolicies.join(', ') : 'None',
  ...
  exportToExcelFile({ "Pull Requests": data }, "AzureDevOps_PullRequests_Policies");
}