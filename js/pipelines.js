async function fetchPipelineData() {
  ...
  try {
    const modernUrl = `${project}/_apis/pipelines?api-version=${API_VERSION}`;
    const classicUrl = `${project}/_apis/build/definitions?api-version=${API_VERSION}`;

    const [modernRes, classicRes] = await Promise.allSettled([
      fetchAzDo(modernUrl),
      fetchAzDo(classicUrl)
    ]);
    ...
    function parseBranch(bObj) {
      ...
      if (!directBranch) return 'Unknown Branch';
    ...
    function cleanIdentity(value) {
      ...
      if (blocked.some(x => text.toLowerCase().includes(x))) return '';
      return text;
    }
    ...
    function parseAuthor(bObj, pipeName, triggerType) {
      ...
      for (const candidate of candidates) {
        if (candidate && candidate.toLowerCase() !== (pipeName || '').toLowerCase()) {
          return candidate;
        }
      }
    ...
    async function getBuildDetails(buildId) {
      ...
      const url = `${project}/_apis/build/builds/${encodeURIComponent(buildId)}?api-version=7.1`;
      try {
        return await fetchAzDo(url);
      } catch (err) {
        return null;
      }
    }
    ...
    async function getPipelineRunDetails(pipelineId, runId) {
      ...
      const url = `${project}/_apis/pipelines/${encodeURIComponent(pipelineId)}/runs/${encodeURIComponent(runId)}?api-version=7.1`;
      try {
        return await fetchAzDo(url);
      } catch (err) {
        return null;
    ...
    for (let i = 0; i < pipeList.length; i += BATCH_SIZE) {
      ...
      await Promise.all(batch.map(async (pipe) => {
        try {
          let runsObtained = [];
          const bUrl = `${project}/_apis/build/builds?definitions=${encodeURIComponent(pipe.id)}&$top=${perPipelineRuns}&queryOrder=queueTimeDescending&api-version=7.1`;
          const bData = await fetchAzDo(bUrl);
          ...
          if (runsObtained.length === 0) {
            const rUrl = `${project}/_apis/pipelines/${encodeURIComponent(pipe.id)}/runs?$top=${perPipelineRuns}&api-version=7.1`;
            const rData = await fetchAzDo(rUrl);
            ...
            runsObtained = await Promise.all(
              rawYamlRuns.map(async (yr) => {
                ...
                const run = detail || yr;
                const selfRepo = run?.resources?.repositories?.self || Object.values(run?.resources?.repositories || {})[0] || null;

                return {
                  id: run.id,
                  buildNumber: run.name || `#${run.id}`,
                  sourceBranch: selfRepo?.refName || selfRepo?.version || run.variables?.['Build.SourceBranch']?.value,
                  reason: run.variables?.['Build.Reason']?.value || run.reason || 'manual',
          ...
          console.warn(`Error processing pipeline ${pipe.name}:`, err);
        }
      }));
    }
